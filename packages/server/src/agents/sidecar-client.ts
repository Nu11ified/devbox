...
export class SidecarHttpClient implements SidecarClient {
  ...
  async gitDiff(cwd?: string): Promise<string> {
    ...
    const res = await fetch(`${this.baseUrl}/git/diff${params}`);
  ...
  async gitApply(patch: string, cwd?: string): Promise<{ success: boolean }> {
    const res = await fetch(`${this.baseUrl}/git/apply`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async ptyStart(
    ...
    const res = await fetch(`${this.baseUrl}/pty/start`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async ptyWrite(sessionId: string, data: string): Promise<void> {
    await fetch(`${this.baseUrl}/pty/write`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async ptyKill(sessionId: string): Promise<void> {
    await fetch(`${this.baseUrl}/pty/kill`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
